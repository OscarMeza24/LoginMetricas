...
export const LOGIN_MUTATION = gql`
  mutation Login($input: LoginInput!) {
    login(input: $input) {
      ...
      message
      accessToken
      user { id email username fullName role isActive }
    }
  }
`;
...
export const CHANGE_PASSWORD_MUTATION = gql`
  mutation ChangePassword($userId: Int!, $input: ChangePasswordInput!) {
    changePassword(userId: $userId, input: $input) { success message }
  }
`;
...
export const VERIFY_TOKEN_QUERY = gql`
  query VerifyToken($token: String!) {
    verifyToken(token: $token) { valid userId message }
  }
`;
...
export const GET_USER_QUERY = gql`
  query GetUser($userId: Int!) {
    getUser(userId: $userId) {
      id email username fullName role isActive createdAt
    }
  }
`;
...
export const LIST_USERS_QUERY = gql`
  query ListUsers($skip: Int, $limit: Int) {
    listUsers(skip: $skip, limit: $limit) {
      total
      users { id email username fullName role isActive createdAt }
    }
  }
`;
